...
public class ChoixResource {
    ...
    @GetMapping("/choix/{questionId}")
    @CrossOrigin("*")
    public ResponseEntity<Choix> getQuestionsAnswers(@PathVariable("questionId") Long questionId){
        String idQ = Long.toString(questionId);
        Optional<Choix> listChoix = choixService.getQA(idQ);
        if (!listChoix.isPresent()) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        return new ResponseEntity<>(listChoix.get(), HttpStatus.OK);
    }
